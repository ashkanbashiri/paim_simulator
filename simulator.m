...
elseif(policyNumber==4)
    ...
    [fcpv,handles.F,cc,p,var,vDelay,pDelay,tv,tvc] = AIM_Optimal2('pdm',2,seed,g,maxSize,handles.spawnRate,handles.simTime,handles.simSpeed,handles);
    %     vDelay
    %     var
    %     cc
...
fcpv=zeros(3,3);
expCounter = 0;
trafficFlows = [300 400 500 600];

for maxSize=1:5
    counter=0;
    for j = [300 400 500 600]%low, medium and high traffic flow
        %seed = seed + 1;
        %expCounter = expCounter+1;
        %trafficFlows = [trafficFlows j];
        counter = counter+1;
        [fcpv(expCounter+1,counter),F,callCounter(expCounter+1,counter),p,var(expCounter+1,counter),vDelay,pDelay,tv,tvc] = trafficLight('pvm',2,seed,g,maxSize,j,handles.simTime-100,handles.simSpeed,handles);
        ...
        tv_sum = sum(tv);
        tvc_sum = sum(tvc);
        capacities(expCounter+1,counter) = tvc_sum*3600/(handles.simTime-100);
